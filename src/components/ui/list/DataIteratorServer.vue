...
export default {
  ...
  setup () {
    ...
    return { v$: useVuelidate(), useResource: useResource(), lgAndUp, mdAndUp }
  },
  validations() {
  ...
  methods: {
    ...
    async onAction(action, item = null, disableRowEdit = null) {
       ...
       * Get freshed item
       */
      this.useResource.setResource(this.listState.resource);
      let { data } = await this.useResource.getOne({id: item.id});
      this.useResource.setItem(data);
      /**
